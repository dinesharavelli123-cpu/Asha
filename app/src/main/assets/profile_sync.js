(()=>{
  ...
  function apply(){
    ...
    const prefs=read(PREFS,{});prefs.profileName=p.name||prefs.profileName||'SAHAAY User';prefs.language=p.language||prefs.language||'English';write(PREFS,prefs);
    const realName=p.name||'SAHAAY User',shownName=realName,initial=(realName.trim().charAt(0)||'S').toUpperCase();
    setText(q('#profileName'),shownName);setText(q('#profileAvatar'),initial);setText(q('#homeAvatar'),initial);
    setText(q('#profileSub'),'SAHAAY ID • '+p.id);
    ...
    const age=ageFromDob(p.dob),ageRow=q('#profileAgeRow');if(ageRow)setText(ageRow.querySelector('small'),age===null?'Not provided':age+' years');
    ...
    const now=new Date().getHours(),hello=now<12?'Good morning':now<17?'Good afternoon':'Good evening';setText(q('#greet'),hello+', '+realName.split(/\s+/)[0]);
    const note=q('#profile .privacy-note');if(note)note.innerHTML='🔐 <b>Tell Once privacy.</b> Your profile, preferences and check-in context stay on this device in this prototype so SAHAAY AI can remember context without repeated retelling.';
  }
