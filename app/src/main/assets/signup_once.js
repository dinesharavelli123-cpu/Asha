(()=>{
  ...
  const applyExisting=()=>{
    try{
      ...
      prefs.anonymous=!!p.anonymous;
      savePrefs(prefs);
      const homeName=document.querySelector('#profileName'); if(homeName) homeName.textContent=p.anonymous?'Anonymous User':prefs.profileName;
    }catch(e){}
  };
  ...
  wrap.innerHTML=`<div class="asha-signup-card"><div class="asha-signup-logo">A</div><h1>Welcome to ASHA</h1><p>Create your local profile once. After this, ASHA will remember you on this device and open directly without asking again.</p><div class="asha-field"><label>NAME OR NICKNAME</label><input id="ashaSignupName" maxlength="40" placeholder="What should ASHA call you?" autocomplete="name"></div><div class="asha-field"><label>PREFERRED LANGUAGE</label><select id="ashaSignupLang"><option>English</option><option>Hindi</option><option>Telugu</option><option>Tamil</option><option>Bengali</option><option>Marathi</option></select></div><div class="asha-field"><label>AGE GROUP</label><select id="ashaSignupAge"><option value="under18">Under 18</option><option value="18-24">18–24</option><option value="25-39">25–39</option><option value="40plus">40+</option><option value="prefer-not">Prefer not to say</option></select></div><label class="asha-anon"><input id="ashaSignupAnon" type="checkbox"><span><b>Start in Anonymous Mode</b><small>Your chosen name stays on this device, but the app can display “Anonymous User”.</small></span></label><div class="asha-local-note">🔒 This demo stores your profile and app history locally on this device. It is not sent to GitHub or a cloud database.</div><button class="asha-sign-btn" id="ashaSignupContinue">Continue to ASHA ✨</button><div class="asha-sign-error" id="ashaSignupError"></div></div>`;
  ...
  const finish=()=>{
    ...
    const profile={name:name||'Anonymous User',language:document.querySelector('#ashaSignupLang').value,ageGroup:document.querySelector('#ashaSignupAge').value,anonymous,createdAt:Date.now()};
    try{
    ...
    setTimeout(()=>{wrap.remove();try{window.dispatchEvent(new Event('asha-profile-updated'));}catch(e){}},280);
  };
