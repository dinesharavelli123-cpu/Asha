...
function renderResult(score,textRisk){
  ...
  $('#explain').textContent=parts.length?`Why SAHAAY changed the signal: ${parts.join(', ')}. The local model combines these factors instead of relying on one answer.`:'Why SAHAAY kept the signal steady: no single factor showed a large enough shift to dominate the result.';
  ...
  $('#summary').textContent=score<65?'Your latest check-in shows a change worth watching. SAHAAY can explain the contributing signals.':'Your latest check-in looks relatively steady. Keep checking in so SAHAAY can build your local pattern.';
